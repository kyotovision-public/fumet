...
class DilatedConv(nn.Module):
    ...
    def __init__(self, dim, k, dilation=1, stride=1, drop_path=0.0, layer_scale_init_value=1e-6, expan_ratio=6):
        """
...
class LGFI(nn.Module):
    ...
    def __init__(
        self, dim, drop_path=0.0, layer_scale_init_value=1e-6, expan_ratio=6, use_pos_emb=True, num_heads=6, qkv_bias=True, attn_drop=0.0, drop=0.0
    ):
        super().__init__()
        ...
        self.gamma_xca = nn.Parameter(layer_scale_init_value * torch.ones(self.dim), requires_grad=True) if layer_scale_init_value > 0 else None
        ...
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones((self.dim)), requires_grad=True) if layer_scale_init_value > 0 else None
        self.drop_path = DropPath(drop_path) if drop_path > 0.0 else nn.Identity()
