...
class TrainerKITTI(TrainerBase):
    def __init__(self, opts):
        if opts.initial_cam_height is not None and opts.first_update_epoch is None:
            print("Using fixed camera height with --initial_cam_height")

        if opts.initial_cam_height is not None:
            self.prev_cam_height_dict = {scale: opts.initial_cam_height for scale in opts.scales}
            self.does_prev_cam_height_dict_exist = True
        else:
            self.does_prev_cam_height_dict_exist = False
        super().__init__(opts)  # when resuming training, self.does_prev_cam_height_dict_exist is set to True in super().__init__()
    ...
    def train(self):
        ...
        for self.epoch in range(self.epoch, self.opt.num_epochs):
            ...
            epoch = self.epoch
            if self.opt.initial_cam_height is not None:
                if self.opt.first_update_epoch is not None and self.epoch >= self.opt.first_update_epoch:
                    new_cam_height_dict = {
    ...
    def compute_cam_heights_and_road_normals(
        ...
        for batch_idx in range(bs):
            ...
            if requires_loss and self.does_prev_cam_height_dict_exist:
